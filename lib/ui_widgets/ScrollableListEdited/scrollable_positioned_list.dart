...
import 'package:yhwh/ui_widgets/ScrollableListEdited/item_positions_notifier.dart';
import 'package:yhwh/ui_widgets/ScrollableListEdited/positioned_list.dart';
import 'package:yhwh/ui_widgets/ScrollableListEdited/post_mount_callback.dart';
...
///
/// [NewScrollablePositionedList] lays out children in the same way as [ListView].
///
...
class NewScrollablePositionedList extends StatefulWidget {
  /// Create a [NewScrollablePositionedList] whose items are provided by
  /// [itemBuilder].
  const NewScrollablePositionedList.builder({
    this.myScrollController,
  ...
  const NewScrollablePositionedList.separated({
    this.myScrollController,
    @required this.itemCount,
    @required this.itemBuilder,
  ...
  ///
  /// Note that the [NewScrollablePositionedList] uses two lists to simulate long
  ...
  @override
  State<StatefulWidget> createState() => _NewScrollablePositionedListState();
}

/// Controller to jump or scroll to a particular position in a
/// [NewScrollablePositionedList].
class ItemScrollController {
  /// Whether any NewScrollablePositionedList objects are attached this object.
  ///
  /// If `false`, then [jumpTo] and [scrollTo] must not be called.
  bool get isAttached => _scrollableListState != null;

  _NewScrollablePositionedListState _scrollableListState;
  ...
  void _attach(_NewScrollablePositionedListState scrollableListState) {
    assert(_scrollableListState == null);
...
class _NewScrollablePositionedListState extends State<NewScrollablePositionedList>
  ...
  @override
  void didUpdateWidget(NewScrollablePositionedList oldWidget) {
  ...
  void _jumpTo({@required int index, double alignment}) {
    ...
    if (_showFrontList) {
      widget.myScrollController.jumpTo(0);
      setState(() {
        frontTarget = index;
  ...
  Future<void> _startScroll(
    ...
    final startingScrollController =
        _showFrontList ? widget.myScrollController : backScrollController;
    ...
    if (itemPosition != null) {
      ...
      final endingScrollController =
          _showFrontList ? backScrollController : widget.myScrollController;
  ...
  void _cancelScroll(_ListDisplay startingListDisplay) {
    widget.myScrollController.jumpTo(widget.myScrollController.offset);
